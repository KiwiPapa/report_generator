...
def check(document):
    ...
    for table in document.tables:
        for row in range(len(table.rows)):
            view_bar(row, len(table.rows))  # 进度条
            for col in range(len(table.columns)):
...
##########################
# 液面高度的获取
if gui.ccbox("请确认：液面高度是否等于评价开始深度？", choices=('不等于', '等于')):
...
if formation_be_or_not == '有储层':
    ...
    for row in range(1, rows3 + 1):
        ...
        if (formation_End <= float(end_Evaluation)) & (formation_Start >= float(start_Evaluation)):
            ...
            evaluation_of_formation2 = layer_evaluation2(df2, formation_Start, formation_End)[1]  # 调取二界面评价函数
...
if formation_be_or_not == '有储层':
    ...
    for row in range(1, rows4 + 1):
        ...
        if (float(formation_Start) <= float(end_Evaluation)) & (float(formation_Start_Depth) >= float(start_Evaluation)):
            ...
            ratio_Series2 = layer_evaluation2(df2, float(formation_Start_Depth), float(formation_Start))[0]  # 调取二界面评价函数
            evaluation_of_formation_upper2 = layer_evaluation2(df2, float(formation_Start_Depth), float(formation_Start))[1]  # 调取二界面评价函数
...
################################################################################
    ...
    for pic_number in range(count):
        ...
        elif all_evaluation_of_formation_upper1[pic_number] in ['中到好，以好为主', '中到好，以中等为主']:
            cbl_value = '中到低'
            case_Wave_Energy = '较强到较弱'
            formation_Wave_Energy = '较弱到较强'
        elif all_evaluation_of_formation_upper1[pic_number] in ['中到差，以差为主', '中到差，以中等为主']:
            cbl_value = '中到高'
            case_Wave_Energy = '较强到强'
            formation_Wave_Energy = '较弱到弱'
        elif all_evaluation_of_formation_upper1[pic_number] in ['好到差，以好为主', '好到差，以差为主']:
            cbl_value = '低到高'
            case_Wave_Energy = '强到弱'
            formation_Wave_Energy = '弱到强'
        elif all_evaluation_of_formation_upper1[pic_number] in ['好到中到差']:
            cbl_value = '低到高'
            case_Wave_Energy = '较强到较弱'
            formation_Wave_Energy = '较弱到较强'
        ###
        ...
        for formation_Number_Temp in range(int(formation_StartNumber), int(formation_EndNumber) + 1):
            if formation_Number_Temp <= len(all_evaluation_of_formation1):
                if all_evaluation_of_formation1[formation_Number_Temp - 1] == '好':
                    ...
                    case_Wave_Energy = '强'
                    formation_Wave_Energy = '弱'
                elif all_evaluation_of_formation1[formation_Number_Temp - 1] in ['中到好，以好为主', '中到好，以中等为主']:
                    cbl_value = '中到低'
                    case_Wave_Energy = '较强到较弱'
                    formation_Wave_Energy = '较弱到较强'
                elif all_evaluation_of_formation1[formation_Number_Temp - 1] in ['中到差，以差为主', '中到差，以中等为主']:
                    cbl_value = '中到高'
                    case_Wave_Energy = '较强到强'
                    formation_Wave_Energy = '较弱到弱'
                elif all_evaluation_of_formation1[formation_Number_Temp - 1] in ['好到差，以好为主', '好到差，以差为主']:
                    cbl_value = '低到高'
                    case_Wave_Energy = '强到弱'
                    formation_Wave_Energy = '弱到强'
                elif all_evaluation_of_formation1[formation_Number_Temp - 1] in ['好到中到差']:
                    cbl_value = '低到高'
                    case_Wave_Energy = '较强到较弱'
                    formation_Wave_Energy = '较弱到较强'
            else:
